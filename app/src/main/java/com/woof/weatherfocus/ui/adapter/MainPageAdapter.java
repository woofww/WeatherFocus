...
public class MainPageAdapter extends AnimRecyclerViewAdapter<RecyclerView.ViewHolder> {
        ...
        switch (viewType){
            ...
            case TYPE_DAYS:
                return new DaysViewHolder(LayoutInflater.from(mContext).
                        inflate(R.layout.forecast_info_item, parent, false));
        }
        return null;
    }
    ...
    /**
     * 可以通过三目运算符对数据返回的status进行判断,此处暂定位固定数值
     * @return
     */
    @Override
    ...
    private class MainViewHolder extends RecyclerView.ViewHolder {
         ...
         * bind方法主要是实现view试图的绑定，完成view的部分
         * @param weather
         */
        public void bind(WeatherEntity weather) {
            ...
            GlideHelper.imageLoad(itemView.getContext(), SharedPreferenceUtil.getInstance().
                    getInt(weather.now.cond.txt, R.mipmap.none), weatherIcon);
        }
    }
    ...
    private class DaysViewHolder extends RecyclerView.ViewHolder {
        int size = weatherData.dailyForecast.size();
        ...
        public void bind(WeatherEntity weather) throws Exception {
            ...
            for (int i = 0; i < size; i++){
                ...
                GlideHelper.imageLoad(
                        ...
                        getInt(weather.dailyForecast.get(i).cond.txtD, R.mipmap.none),
                        forecastIcon[i]);
